...
shaka.util.StreamUtils.chooseCodecsAndFilterManifest = function(
  ...
  /**
   * Get minimal count of variants by codec.
   * Average bandwidth of H.265 is higher than H.264.
   * H.265 has 4K support and more variants.
   * @param {!Array.<shaka.extern.Variant>} variants
   * @param {number} count
   * @returns {!Array.<shaka.extern.Variant>}
   */
  function getMinCountVariantsByCodec(variants, count) {
    return variants
  ...
  let minCountVariantsByCodec = Infinity;
  variantsByCodecs.forEach((_, variants) => {
    if (variants.length && variants.length < minCountVariantsByCodec) {
      minCountVariantsByCodec = variants.length;
    }
  });

  goog.asserts.assert(!isNaN(minCountVariantsByCodec),
      'minCountVariantsByCodec should be a number!');

  variantsByCodecs.forEach((codecs, variants) => {
    let sum = 0;
    let num = 0;
    for (const variant of getMinCountVariantsByCodec(variants, minCountVariantsByCodec)) {
