    ...
    if (isReadableStreamSupported && isMP4) {
      ...
      const streamDataCallback = async (data) => {
        ...
        if (sawMDAT) {
          ...
          lastAppend = this.append_(mediaState, presentationTime, currentPeriod, stream, reference, dataToAppend);
        }
      };


      await this.fetch_(mediaState, stream, reference, streamDataCallback, onRetry);
      await lastAppend;
    } else {
