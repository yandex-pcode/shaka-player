...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = async function(
  ...
  const isReadableStreamSupported = shaka.net.HttpFetchPlugin.isSupported();

  try {
    if (isReadableStreamSupported && isMP4) {
      ...
      const streamDataCallback = async (data) => {
        ...
        try {
          ...
          if (this.destroyed_ || this.fatalError_) return;
          appendQueue.push(this.append_(mediaState, presentationTime, currentPeriod, stream, reference, dataToAppend));
        } catch (e) {}
      };
      ...
      if (appendQueue.length === 0) {
        // if streamDataCallback was not fired append full segment manually
        await this.append_(mediaState, presentationTime, currentPeriod, stream, reference, fullSegment);
      }
    } else {
      let fetchSegment = this.fetch_(mediaState, stream, reference);
      const results = await Promise.all([initSourceBuffer, fetchSegment]);
      if (this.destroyed_ || this.fatalError_) return;

      await this.append_(mediaState, presentationTime, currentPeriod, stream, reference, results[1]);
    }
