 ...
 */
shaka.media.StreamingEngine.prototype.getNewSegment_ =
...
shaka.media.StreamingEngine.prototype.shouldAbortCurrentRequest_ =
  function(mediaState, periodIndex) {
    ...
    if (presentationTime > newSegmentStartTime) {
      // do not abort if we already started playing new segment
      return false;
    }
...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  if (reference.getUris()[0].includes(('v1-x2')))
    console.log('fetch', reference.getUris()[0]);

  this.fetch_(mediaState, reference, async (time, bytes, numBytesRemaining, data) => {
    ...
    if (reference.getUris()[0].includes(('v1-x2')))
      console.log('fetched', reference.getUris()[0]);
 ...
 *   reference
 * @param {shaka.extern.ProgressUpdated=} dataUpdated
 * @return {!Promise.<!ArrayBuffer>}
 * @private
 */
shaka.media.StreamingEngine.prototype.fetch_ = function(mediaState, reference, dataUpdated) {
  ...
  const op = this.playerInterface_.netEngine.request(requestType, request, dataUpdated);
