...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  const queueAppend = (data) => {
    if (this.destroyed_ || this.fatalError_ || !data) {
      return;
    }

    shaka.log.v2(
      logPrefix,
      `#${reference.position} appending ${data.byteLength} bytes`
    );
    appendQueue = appendQueue.then(() => this.append_(mediaState,
      presentationTime,
      currentPeriod,
      stream,
      reference,
   ...
   */
  const onDataChunkLoaded = (time, bytes, numBytesRemaining, value) => {
    if (this.destroyed_ || this.fatalError_ || !value || !value.byteLength) return;

    const newData = new Uint8Array(value);
    // concat remaining + newData
    if (remaining.length === 0) {
      remaining = newData;
    } else {
      const tmp = new Uint8Array(remaining.length + newData.length);
      tmp.set(remaining, 0);
      tmp.set(newData, remaining.length);
      remaining = tmp;
    }

    const boxesInfo = shaka.media.StreamingEngine.findLastTopIsoBoxCompleted_(['moov', 'mdat'], remaining, offset);
    if (boxesInfo.found) {
  ...
  this.fetch_(mediaState, reference, onDataChunkLoaded).then(function(results) {
    ...
    shaka.log.v1(logPrefix, `finished fetch and append #${reference.position}`);
  }.bind(this)).catch(function(error) {
    if (this.destroyed_ || this.fatalError_) return;
 ...
 * @private
 */
shaka.media.StreamingEngine.parseUint32_ = (data, offset) => {
 ...
 * @private
 */
shaka.media.StreamingEngine.parseIsoBoxType_ = (data, offset) => {
 ...
 * @typedef {{
  ...
  * }}
  */
shaka.media.StreamingEngine.IsoBoxSearchInfo;
 ...
 * @param {number=} offset
 * @returns {shaka.media.StreamingEngine.IsoBoxSearchInfo}
 * 
 * @private
 */
shaka.media.StreamingEngine.findLastTopIsoBoxCompleted_ = (types, buffer, offset = 0) => {
  ...
  while (offset < data.byteLength) {
      const boxSize = shaka.media.StreamingEngine.parseUint32_(data, offset);
      const boxType = shaka.media.StreamingEngine.parseIsoBoxType_(data, offset + 4);
