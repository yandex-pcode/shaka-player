...
shaka.media.StreamingEngine.prototype.switchInternal_ = function(
  ...
  const newSegment = this.getNewSegment_(mediaState, periodIndex);
  if (this.shouldAbortCurrentRequest_(mediaState, periodIndex)) {
    ...
    // if segment was aborted, we should flush buffer
    // in case it was partially downloaded
    const currentPeriod = this.manifest_.periods[periodIndex];

    if (newSegment) {
        const startTime = newSegment.startTime + currentPeriod.startTime;
        const endTime = newSegment.endTime + currentPeriod.startTime;
        this.playerInterface_.mediaSourceEngine.remove(
            mediaState.type, startTime, endTime);
    }
  }
 ...
 */
shaka.media.StreamingEngine.prototype.getNewSegment_ = function(
  mediaState, periodIndex
) {
  // If the operation is completed, it will be set to null, and
  // new segment is no available
  ...
  // we just switched.
    return this.getSegmentReferenceNeeded_(
 ...
 * @return {boolean}
 */
shaka.media.StreamingEngine.prototype.shouldAbortCurrentRequest_ = function(
  mediaState, periodIndex
) {
  const newSegment = this.getNewSegment_(mediaState, periodIndex);
  if (!newSegment) {
  ...
  const newSegmentStartTime =
    newSegment.getStartTime() + currentPeriod.startTime;
  const presentationTime = this.playerInterface_.getPresentationTime();
  if (presentationTime > newSegmentStartTime) {
    // do not abort if it was partially downloaded and
    // we already started playing new segment
    return false;
  }
...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  const logPrefix = StreamingEngine.logPrefix_(mediaState) + `#${reference.position}`;
  ...
  /**
   * @type {function(ArrayBuffer, boolean)} 
   * Appends next data chunk to mp4box and increments nextFileStart
   */
  const appendDataChunkToMP4Box = (data, last) => {
    data['fileStart'] = nextFileStart;
    nextFileStart = mp4BoxFile['appendBuffer'](data, last);
  };

  let nextEmsgBoxIndex = 0;
  // loops parsed boxes and parses emsg
  const parseEmsgBoxes = () => {
    const { emsgSchemeIdUris } = stream;
    const streamHasEmsg = emsgSchemeIdUris != null && emsgSchemeIdUris.length > 0;
    if (!streamHasEmsg) {
      return;
    }

    const boxes = mp4BoxFile['boxes'] || [];
    while (nextEmsgBoxIndex < boxes.length) {
      // check if next box is emsg
      const box = boxes[nextEmsgBoxIndex];
      nextEmsgBoxIndex++;

      if (box.type === 'emsg') {
        this.parseEMSG_(currentPeriod, reference, emsgSchemeIdUris, box);
      }
    }
  };
  ...
  mp4BoxFile['onReady'] = ({ tracks }) => {
    mp4BoxFile['setSegmentOptions'](tracks[0].id, null, { 'nbSamples': 1 });
  ...
  mp4BoxFile['onSegment'] = (id, user, buffer) => {
    ...
    const queueAppend = () => {
      ...
      if (stream.encrypted) {
        // do not push partial segments for encrypted stream
        return Promise.resolve();
      }
    ...
    // queue new chunk and wait for previous append if any
    appendQueue = appendQueue.then(queueAppend);
  };
  
  // store promise queue for data appending
  // to keep the append order
  // append only after source buffer is ready
  let appendQueue = initSourceBuffer.then(() => {
    const { data } = mediaState.stream.initSegmentReference;
    if (!data) {
      return;
    }
    appendDataChunkToMP4Box(data, false);
  });

  /**
   * @type {shaka.extern.ProgressUpdated}
   */
  const onDataChunkLoaded = (time, bytes, numBytesRemaining, data) => {
    if (this.destroyed_ || this.fatalError_ || !data) return;

    shaka.log.v2(
      logPrefix,
      `#${reference.position} appending ${data.byteLength} bytes to mp4box, last=${numBytesRemaining === 0}`
    );
    const { buffer } = (/** @type {!Uint8Array|ArrayBuffer} */(data));
    initSourceBuffer.then(() => {
      appendDataChunkToMP4Box(buffer, numBytesRemaining === 0);
    });
  };

  this.fetch_(mediaState, reference, onDataChunkLoaded)
  .then((fullSegment) => {
    if (this.destroyed_ || this.fatalError_) return;
    return appendQueue.then(() => fullSegment);
  }).then((fullSegment) => {
    if (stream.encrypted) {
      return this.append_(mediaState,
        presentationTime,
    ...
    // update lastSegmentReference and stream when segment is fully loaded
...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  let appendInit = fetchInit.then(function(initSegment) {
    if (this.destroyed_) return;
    mediaState.stream.initSegmentReference.data = initSegment;
...
shaka.media.StreamingEngine.prototype.parseEMSG_ = function(
  ...
  let schemeId = box['scheme_id_uri'];
  let value = box['value'];
  let timescale = box['timescale'];
  let presentationTimeDelta = box['presentation_time_delta'];
  let eventDuration = box['event_duration'];
  let id = box['id'];
  let messageData = box['message_data'];
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.fetch_ = function(
  mediaState, reference, dataChunkLoaded
) {
  ...
  const op = this.playerInterface_.netEngine.request(
    requestType, request, dataChunkLoaded);
