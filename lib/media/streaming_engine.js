...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  let offset = 0;
  let fetchSegment = this.fetch_(mediaState, reference, (loadedData) => {
    const boxesInfo = shaka.media.StreamingEngine.findLastTopIsoBoxCompleted_(['moov', 'mdat'], loadedData, offset);
    if (!boxesInfo.found) {
      return;
    }
    const end = boxesInfo.lastCompletedOffset + boxesInfo.size;
    const loadedBoxes = loadedData.subarray(offset, end);
    queueAppend(loadedBoxes);
    offset = end;
  });
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.fetch_ = function(mediaState, reference, onPartialLoad) {
  ...
  const op = this.playerInterface_.netEngine.request(requestType, request, onPartialLoad);
 ...
 * @private
 */
shaka.media.StreamingEngine.findLastTopIsoBoxCompleted_ = (types, data, offset = 0) => {
