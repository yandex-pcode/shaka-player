 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = async function(
  ...
  const isDestroyed = () => this.destroyed_ || this.fatalError_;

  /** @type {Array<Promise>!} */
  let appendQueue = [];
  let fullSegmentFetched = false;
  let attempt = 0;
  let bytesAppended = 0;
  let remaining = new Uint8Array(0);

  const append = async (data, isFullSegment) => {
    const currentAttempt = attempt;
    await initSourceBuffer;
    await Promise.all(appendQueue);

    if (isDestroyed() || fullSegmentFetched && !isFullSegment) return;
    if (currentAttempt !== attempt) return;

    if (isFullSegment) {
      // full segment was already appended
      if (bytesAppended === data.byteLength) return;
    } else {
      bytesAppended += data.byteLength;
    }

    const appendTask = this.append_(mediaState, presentationTime, currentPeriod, stream, reference, data);
    appendQueue.push(appendTask);
    return appendTask;
  };


  const streamDataCallback = (data) => {
    ...
    append(dataToAppend, /* isFullSegment */ false).catch(() => {});
  };
  ...
  try {
    const fullSegment = await this.fetch_(mediaState, stream, reference, streamDataCallback, onRetry);
    if (isDestroyed()) return;

    fullSegmentFetched = true;
    await append(fullSegment, /* isFullSegment */ true);
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.concatArray_ = function(remaining, data) {
...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  let fetchInit =
      this.fetch_(mediaState, mediaState.stream, mediaState.stream.initSegmentReference);
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.fetch_ = function(mediaState, stream, reference, streamDataCallback, onRetry) {
