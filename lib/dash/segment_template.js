...
shaka.dash.SegmentTemplate.createFromIndexTemplate_ = function(
  ...
  let baseUris = ManifestParserUtils.resolveUris(
      context.baseUris.baseUris_,
      context.representation.baseUris);

  let resolvedUris = ManifestParserUtils.resolveUris(baseUris, [filledTemplate]);
...
shaka.dash.SegmentTemplate.createFromTimeline_ = function(context, info) {
  ...
  for (let i = 0; i < info.timeline.length; i++) {
    ...
    let createUris = (function(
            template, repId, bandwidth, ctxBaseUris, reprBaseUris, segmentId, time) {
          let baseUris = ManifestParserUtils.resolveUris(
              ctxBaseUris.baseUris_,
              reprBaseUris
          );

          let mediaUri = MpdUtils.fillUriTemplate(template, repId, segmentId, bandwidth, time);

          return ManifestParserUtils.resolveUris(baseUris, [mediaUri])
              .map(function(g) { return g.toString(); });
        }.bind(null, info.mediaTemplate, context.representation.id,
               context.bandwidth || null, context.baseUris,
               context.representation.baseUris,
