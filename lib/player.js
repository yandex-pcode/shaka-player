 ...
 * @return {Promise<?shaka.extern.Variant>}
 * @private
 */
...
shaka.Player.prototype.chooseVariantAndSwitch_ =
    async function(period, fireAdaptationEvent = true) {
  goog.asserts.assert(this.config_, 'Must not be destroyed');
  ...
  const chosenVariant = await this.chooseVariant_(period.variants);
  let changed = false;
  if (chosenVariant) {
 ...
 *
 * @param {!shaka.extern.Period} period
 * @return {Promise<shaka.media.StreamingEngine.ChosenStreams>}
 *    An object containing the chosen variant and text stream.
 * @private
 */
shaka.Player.prototype.onChooseStreams_ = async function(period) {
  ...
  try {
    ...
    const chosen = await this.chooseStreams_(period);
 ...
 * @private
 */
shaka.Player.prototype.chooseStreams_ = async function(period) {
  ...
  let chosenVariant = await this.chooseVariant_(period.variants);
