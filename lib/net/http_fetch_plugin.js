...
shaka.net.HttpFetchPlugin.request_ = async function(
    uri, requestType, init, abortStatus, progressUpdated, streamDataCallback) {
  ...
  let gotError = false;
  const isPending = () => !gotError && !abortStatus.canceled && !abortStatus.timedOut;
    ...
    let start = (controller) => {
      let push = async () => {
        ...
        if (!readObj.done) {
          ...
          if (streamDataCallback && isPending()) {
