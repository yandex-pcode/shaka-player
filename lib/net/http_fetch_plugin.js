...
shaka.net.HttpFetchPlugin.request_ = async function(
  ...
  const isLoadCompleted = () => loadCompleted || contentLength > 0 
    && request.loadedData 
    && request.loadedData.length === contentLength;
    ...
    if (response.ok && requestType === shaka.net.NetworkingEngine.RequestType.SEGMENT) {
      let start = () => {
        let readNextChunk = async () => {
          ...
          progressUpdated(currentTime - lastTime, value.length, contentLength - request.loadedData.length, request.loadedData);
  ...
  const loadedData = loadCompleted && request.loadedData && request.loadedData.length > 0
