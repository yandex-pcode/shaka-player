  ...
  try {
    ...
    let start = (controller) => {
      let push = async () => {
        ...
        if (abortStatus.timedOut || abortStatus.canceled) {
          // do not process data if request was canceled or aborted
          if (readableStream) readableStream.cancel();
          return;
        }
        ...
        const loadTime = currentTime - lastTime;
        const bytesRemaining = contentLength - loaded;

        // notify progress only when status code is 2xx
        if (response.ok && loadedBytes > 0) {
          progressUpdated(loadTime, loadedBytes, bytesRemaining, readObj.value);
        }
    ...
    readableStream = new ReadableStream({start}); // eslint-disable-line no-new
