...
shaka.net.HttpFetchPlugin.request_ = async function(
  ...
  const getLoadedDataLength = () => request.loadedData && request.loadedData.length || 0;

  const isLoadCompleted = () => loadCompleted || segmentSize > 0 
    && getLoadedDataLength() === segmentSize;

  let gotError = false;
  const isAborted = () => abortStatus.canceled || abortStatus.timedOut || gotError;
  ...
  try {
    ...
    contentLength = contentLengthRaw ? parseInt(contentLengthRaw, 10) : 0;
    if (contentLength > 0) {
      segmentSize = contentLength + getLoadedDataLength();
    }

    if (response.ok && requestType === shaka.net.NetworkingEngine.RequestType.SEGMENT) {
      let start = () => {
        let readNextChunk = async () => {
          if (isAborted() || isLoadCompleted()) {
            return;
          }
          let readObj;
          try {
          ...
          const loadedData = request.loadedData || new Uint8Array(0);
          request.loadedData = new Uint8Array(loadedData.length + value.length);
          request.loadedData.set(loadedData);
          request.loadedData.set(value, loadedData.length);
  
          loadCompleted = isLoadCompleted();
          
          let currentTime = Date.now();
          progressUpdated(currentTime - lastTime, value.length, segmentSize - request.loadedData.length, request.loadedData);
          lastTime = currentTime;
  
          if (!isLoadCompleted() && !isAborted()) {
            readNextChunk();
          }
        };
        readNextChunk();
    ...
    if (!isLoadCompleted()) {
      if (abortStatus.canceled) {
        throw new shaka.util.Error(
            ...
            uri, requestType, request.loadedData && request.loadedData.length || 0);
        ...
        throw new shaka.util.Error(
            ...
            uri, error, requestType, request.loadedData && request.loadedData.length || 0);
      }
    }
  }
  ...
  const loadedData = isLoadCompleted() && request.loadedData && request.loadedData.length > 0
    ? request.loadedData.buffer
    : arrayBuffer;
  return shaka.net.HttpPluginUtils.makeResponse(headers,
    loadedData, response.status, uri, response.url, requestType);
};
