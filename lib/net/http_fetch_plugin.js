...
shaka.net.HttpFetchPlugin.request_ = async function(
  ...
  const isLoadCompleted = () => contentLength > 0 
    && request.loadedData 
    && request.loadedData.length === contentLength;

  const isAborted = () => abortStatus.canceled || abortStatus.timedOut;
  ...
  try {
    ...
    loadCompleted = loadCompleted || isLoadCompleted();
    if (!loadCompleted) {
  ...
  const loadedData = request.loadedData && request.loadedData.length > 0
