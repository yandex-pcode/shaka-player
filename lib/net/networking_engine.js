 ...
 * @export
 */
shaka.net.NetworkingEngine.prototype.request = function(
  type, request, dataChunkLoaded
) {
  ...
  let requestOperation = requestFilterOperation.chain(
      () => this.makeRequestWithRetry_(
        type, request, numBytesRemainingObj, dataChunkLoaded
      )
  );
  ...
  const operation = responseFilterOperation.chain((responseAndGotProgress) => {
    ...
    if (!responseAndGotProgress.gotProgress &&
      ...
      if (dataChunkLoaded) {
        dataChunkLoaded(response.timeMs, response.data.byteLength, 0, response.data);
      }
    }
    return response;
  }, (e) => {
...
shaka.net.NetworkingEngine.prototype.makeRequestWithRetry_ =
    function(type, request, numBytesRemainingObj, dataChunkLoaded) {
  ...
  return this.send_(type, request, backoff, index, /* lastError */ null,
                    numBytesRemainingObj, dataChunkLoaded);
};
...
shaka.net.NetworkingEngine.prototype.send_ = function(
    type, request, backoff,
    index, lastError, numBytesRemainingObj, dataChunkLoaded) {
  let uri = new goog.Uri(request.uris[index]);
  let scheme = uri.getScheme();
  // Whether it got a progress event.
  let gotProgress = false;
  // Whether it failed with error.
  let gotError = false;
  if (!scheme) {
  ...
  let sendOperation = backoffOperation.chain(() => {
    ...
    if (request.loadedBytes > 0) {
        request.headers['Range'] = 'bytes=' + request.loadedBytes + '-';
    }
    return plugin(request.uris[index],
         request,
         type,
         // The following function is passed to plugin.
         (time, bytes, numBytesRemaining, data) => {
           if (this.onProgressUpdated_ && type == segment) {
             this.onProgressUpdated_(time, bytes);
             gotProgress = true;
             numBytesRemainingObj.setBytes(numBytesRemaining);
             if (!dataChunkLoaded || !data || !data.byteLength || gotError) {
               return;
             }
             request.loadedBytes += data.byteLength;
             dataChunkLoaded(time, data.byteLength, numBytesRemaining, data);
           }
        });
  }).chain((response) => {
    ...
    if (error && error.severity == shaka.util.Error.Severity.RECOVERABLE) {
      ...
      const requestClone = /** @type {shaka.extern.Request} */(Object.assign({}, request));
      return this.send_(type, requestClone, backoff, index, shakaError,
                        numBytesRemainingObj, dataChunkLoaded);
    }
