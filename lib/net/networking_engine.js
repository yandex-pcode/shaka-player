...
shaka.net.NetworkingEngine.prototype.request = function(type, request, onPartialLoad) {
  ...
  let requestOperation = requestFilterOperation.chain(
      () => this.makeRequestWithRetry_(type, request, numBytesRemainingObj, onPartialLoad));
...
shaka.net.NetworkingEngine.prototype.makeRequestWithRetry_ =
    function(type, request, numBytesRemainingObj, onPartialLoad) {
  ...
  return this.send_(type, request, backoff, index, /* lastError */ null,
                    numBytesRemainingObj, onPartialLoad);
};
...
shaka.net.NetworkingEngine.prototype.send_ = function(
    type, request, backoff, index, lastError, numBytesRemainingObj, onPartialLoad) {
  ...
  let sendOperation = backoffOperation.chain(() => {
    ...
    if (request.loadedData && request.loadedData.length > 0) {
      request.headers['Range'] = 'bytes=' + request.loadedData.length + '-';
    }
    ...
    if (onPartialLoad) {
      onPartialLoad(new Uint8Array(response.data));
    }
