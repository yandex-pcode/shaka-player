 ...
 * @export
 */
shaka.net.NetworkingEngine.prototype.request = function(type, request, dataUpdated) {
  ...
  let requestOperation = requestFilterOperation.chain(
      () => this.makeRequestWithRetry_(type, request, numBytesRemainingObj, dataUpdated));
  ...
  const operation = responseFilterOperation.chain((responseAndGotProgress) => {
    ...
    if (!responseAndGotProgress.gotProgress &&
      ...
      dataUpdated(response.timeMs, response.data.byteLength, 0, response.data);
    }
    return response;
  }, (e) => {
...
shaka.net.NetworkingEngine.prototype.makeRequestWithRetry_ =
    function(type, request, numBytesRemainingObj, dataUpdated) {
  ...
  return this.send_(type, request, backoff, index, /* lastError */ null,
                    numBytesRemainingObj, dataUpdated);
};
...
shaka.net.NetworkingEngine.prototype.send_ = function(
    type, request, backoff, index, lastError, numBytesRemainingObj, dataUpdated) {
    ...
    return plugin(request.uris[index],
         ...
         (time, bytes, numBytesRemaining, data) => {
           if (this.onProgressUpdated_ && type == segment) {
             ...
             dataUpdated(time, bytes, numBytesRemaining, data);
           }
        });
  }).chain((response) => {
